...
# Format the correlation and p-value for display
text_label <- sprintf("r = %.2f, p = %.3f", cor_value, p_value)

# Create a basic scatter plot
p1 <- ggplot(data, aes_string(x = xvar, y = yvar)) +
  geom_point(shape = 21, fill = "gray") +  # Open circles
  geom_smooth(method = "lm", se = TRUE, color = "blue", fill = "lightblue") +
  annotate("text", x = Inf, y = Inf, label = text_label, hjust = 1.1, vjust = 1.1, size = 5, colour = "black") +
  theme_cowplot() +
  labs(x = "Vision SB Update", y = "Vision M-Ratio")+
  scale_x_continuous(breaks = seq(-100, 100, by = 25))+
  scale_y_continuous(limits= c(-0.5,2.5), breaks = seq(-0.5,2.5, by = .5))


# Create the ggExtra plot with marginal histograms
p1a <- ggExtra::ggMarginal(p1, type = "histogram", fill = "grey")

xvar <- "CalorieSBD"
yvar <- "mratio_Calories"
...
# Format the correlation and p-value for display
text_label <- sprintf("r = %.2f, p < 0.001", cor_value)

# Create a basic scatter plot
p2 <- ggplot(data, aes_string(x = xvar, y = yvar)) +
  geom_point(shape = 21, fill = "gray") +  # Open circles
  geom_smooth(method = "lm", se = TRUE, color = "blue", fill = "lightblue") +
  annotate("text", x = Inf, y = Inf, label = text_label, hjust = 1.1, vjust = 1.1, size = 5, colour = "black") +
  theme_cowplot() +
  labs(x = "Calories SB Update", y = "Calories M-Ratio")+
  scale_x_continuous(breaks = seq(-100, 100, by = 25))+
  scale_y_continuous(limits= c(-2,3), breaks = seq(-2, 3, by = 1))# Adjust this as per your data range and preference


# Create the ggExtra plot with marginal histograms
p2a <- ggMarginal(p2, type = "histogram", fill = "grey")



patchwork::wrap_elements(p1a) | patchwork::wrap_elements(p2a)

ggsave(filename = "figures/Figure_4.png", height = 4, width = 8)
ggsave(filename = "figures/Figure_4.tiff", height = 4, width = 8, dpi=300)
